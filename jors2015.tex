...
results. This new e-infrastructure for computational science offers
...
browser) famously stated in 2011 that ``{\emph{software is eating the
...
proposed~\cite{deroure:2010,stodden-et-al:2013,fursin+dubach:2014}. Thus,
the reproduction and replication of reported scientific results has
become a widely discussed topic within the scientific
...
area~\cite{sim-et-al:2003,chirigati-et-al:2013,stodden+miguez:2014},
...
Reproducibility (replication, repeatability) is a fundamental tenet of
...
support sharing of computational artefacts (for example, as number of
high-profile computer science conferences now explicitly acknowledge
the importance of reproducibility, promoting community-driven
reviewing and validation; we have recently proposed something for the
...
API) changes too. This system (see Figure~\ref{fig:workflow}) woud
...
already several web services that nearly do all of this things (for
example, a repository for disseminating the computational models
associated with publications in the social and life
...
standardisation?}}  Such a service may appear non-trivial, given the
large numbers of tools and workflows that could potentially require to
be supported by the service. Furthermore, after such a service has
been implemented, how do we ensure it is \emph{useful} and
...
should aim through software infrastructure and research workflows to
mitigate these costs. Furthermore, we can reasonably expect the
distinct needs of specific research communities to evolve over time,
and initial implementations of the platform may require refinement in
response to user feedback (supporting the vital cultural change by
reducing overheads and improving the efficiency of researchers). As
such, if the wider research community is to move to requiring
reproducibility, it seems most reasonable that this is staggered over
a number of years to allow for both of these elements to develop,
until eventually all researchers are required to use the service.

This proposed new infrastructure could thus have a profound impact on
the way that computational science is performed, repositioning the
role of models, algorithms and benchmarks and accelerating the
research cycle, perhaps truly enabling a ``fourth paradigm'' of data
intensive scientific discovery~\cite{hey:2009}.
